  ...
  const [jobs, setJobs] = useState([
    {
      ...
      job_end: "2022",
      job_bio:
        "I maintained the ICP EU unit and was a a lead Dev ensuring that the team will hit all marks by deadlines",
    },
  ]);
        ...
        <div className="cv_creator">
          <form className="page">
            ...
            <div>
              {schools.map((s) => (
                <SchoolForm
                  handleChange={(e) => handleChange(e, setSchools)}
                  key={s.id}
                  onChange={(e) => handleChange(e, setSchools)}
                  removeEntry={(e) => {
                    removeEntry(s.id, setSchools);
                  }}
                  schoolData={s}
                >
                  {" "}
              ...
              <div
                className="btn"
                onClick={(e) => handleAddClick(setSchools)}
                name="schools"
              >
                {" "}
                Add Education{" "}
              ...
              {jobs.map((j) => (
                <CompanyForm
                  handleChange={(e) => handleChange(e, setJobs)}
                  key={j.id}
                  onChange={(e) => {
                    handleChange(e, setJobs);
                  }}
                  removeEntry={(e) => {
                    removeEntry(j.id, setJobs);
                  }}
                  jobsData={j}
                >
                  {" "}
              ...
              <div
                className="btn"
                onClick={(e) => handleAddClick(setJobs)}
                name="jobs"
              >
                {" "}
                Add Work Experince{" "}
